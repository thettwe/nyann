...
#
# Verbs:
#   start   — launch ci-sentinel.sh --daemon-loop under a platform supervisor
#             (launchd on macOS, systemd USER unit on Linux, nohup fallback),
#             survive terminal close. IDEMPOTENT: a live daemon for the repo
#             is left alone (single-instance via the <repo-hash>.sentinel.pid).
...
#             gone.
#   status  — report pid + supervisor + watched repo/PRs; flags a STALE daemon
#             (pid file present but process dead).
#   restart — stop then start.
#   report  — machine-readable summary of every running daemon (consumed by
#             doctor so a running sentinel is never invisible).
#
