...
if $apply; then
  ...
  #
  # Fully canonicalize the chain — a one-level readlink severs a 2-hop
  # link (README.md → a.md → real.md) at the intermediate hop. macOS's
  # older `readlink` lacks `-f`, so loop manually until the path stops
  # being a symlink, resolving each hop relative to its own directory.
  hops=0
  while [[ -L "$readme" ]]; do
    if (( hops++ > 40 )); then
  ...
  # Guard against an absolute / traversing symlink that resolves OUTSIDE
  # the target tree — without this, README.md → /tmp/outside/x.md would
  # write outside the repo. assert_path_under_target dies on escape.
  ...
  if [[ ! -f "$readme" ]]; then
    ...
    if grep -Fq "$marker_start" "$readme"; then
      ...
      # orphaned marker_start without a matching marker_end (e.g., from
      # a prior partial write). Must check the source — the awk pass
      # injects a new body that always contains marker_end, so checking
      # the output would never catch the orphan.
      ...
      # Refuse when the START marker appears more than once. Two start
      # markers (a duplicated block, or the marker quoted in prose / a
      # code fence) make the block boundaries genuinely ambiguous —
      # rewriting would risk deleting the content between occurrences.
      # Bail rather than corrupt. We deliberately do NOT refuse on a
      # duplicate END marker: a single start marker plus a stray
      # end-marker mention is unambiguous (the state machine closes the
      # block at the first end marker AFTER the start, and prints any
      # end-marker substring before the start verbatim).
      ...
      # replaced so any later marker-substring lines (in prose or a code
      # fence) print verbatim. The end marker is honored only while inside
      # the active block (skip==1) — an end-marker substring before the
      # start marker is ordinary content and printed as-is.
      ...
      # Prepend before the first heading. Terminate with exactly one
      # trailing newline so a first apply (this prepend path) and a
      # second apply (the awk replace path, which ends every line with
      # `\n`) produce byte-identical output even when the original
      # README lacked a trailing newline — i.e. --apply stays idempotent.
      tmp=$(mktemp -t nyann-readme.XXXXXX)
