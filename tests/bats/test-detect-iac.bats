...
@test "terraform-docs hook regenerates README for deeply-nested module" {
  # Regression: modules nested deeper than modules/<name>/ (the layout
  # detect-iac.sh advertises, e.g. modules/aws/networking/) must get their
  # README regenerated — not skipped because of a fixed two-segment depth.
  cd "$REPO" && git init -q -b main && git config user.email t@t && git config user.name t

  # Stub terraform-docs: write a marker README into the module dir it's
  # invoked against (last arg). Proves the hook targeted the deep module.
  stub_bin="$TMP/stub"
  mkdir -p "$stub_bin"
  cat > "$stub_bin/terraform-docs" <<'STUB'
#!/usr/bin/env bash
dir="${!#}"
printf '# generated by terraform-docs\n' > "${dir}/README.md"
exit 0
STUB
  chmod +x "$stub_bin/terraform-docs"

  mkdir -p modules/aws/networking
  echo 'resource "null_resource" "x" {}' > modules/aws/networking/main.tf
  echo "# stale" > modules/aws/networking/README.md
  git add modules/aws/networking/main.tf modules/aws/networking/README.md
  git -c core.hooksPath=/dev/null commit -q -m "feat: initial"

  # Stage a .tf change in the deep module.
  printf '\n# new\n' >> modules/aws/networking/main.tf
  git add modules/aws/networking/main.tf

  PATH="$stub_bin:$PATH" bash "$REPO_ROOT/templates/hooks/iac/terraform-docs.sh" \
    >/dev/null 2>&1

  # README regenerated by the stub …
  grep -q "generated by terraform-docs" modules/aws/networking/README.md
  # … and re-staged.
  staged_now=$( git diff --cached --name-only | sort | tr '\n' ' ' )
  case "$staged_now" in
    *"modules/aws/networking/README.md"*) : ;;
    *) echo "deep module README was not staged: $staged_now" >&2; false ;;
  esac
}
